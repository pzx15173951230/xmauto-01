...
(function () {
  ...
  var Parabola = function Parabola(options) {
  ...
  Parabola.prototype = {
    ...
    initialize: function initialize(options) {
      ...
      this.elOriginalTop = toInteger(this.$el.css("top")); // this.driftX X杞寸殑鍋忕Щ鎬婚噺
      ...
      this.duration = ops.duration; // 澶勭悊鍏紡甯搁噺

      this.curvature = ops.curvature; // 鏍规嵁涓ょ偣鍧愭爣浠ュ強鏇茬巼纭畾杩愬姩鏇茬嚎鍑芥暟锛堜篃灏辨槸纭畾a, b鐨勫€硷級
      //a=this.curvature
      ...
      this.b = (this.driftY - this.curvature * this.driftX * this.driftX) / this.driftX; //鑷姩寮€濮�
    ...
    step: function step(now) {
      ...
      if (now > this.end) {
        ...
        //x 姣忎竴姝ョ殑X杞寸殑浣嶇疆
        x = this.driftX * ((now - this.begin) / this.duration); //姣忎竴姝ョ殑Y杞寸殑浣嶇疆y = a*x*x + b*x + c;   c==0;
